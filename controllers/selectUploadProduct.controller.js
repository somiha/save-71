...
exports.selectUploadProduct = async (req, res) => {
  ...
  if (isLogged) {
    try {
      const notification = await catModel.fetchAllNotifications(crypto.decrypt(req.cookies.userId));
      const currencyCode = crypto.decrypt(req.cookies.currencyCode);
      const [mainCat, subCat, extraCat, allCat, fetchFeaturedImages, tempImages, currRate] = await Promise.all([
      ...
      db.query("SELECT * FROM `orders` INNER JOIN `order_details` ON `orders`.`order_id` = `order_details`.`order_id` INNER JOIN `products` ON `products`.`product_id` = `order_details`.`product_id` WHERE `orders`.`user_id` = ? AND `orders`.`in_cart` = 1 ORDER BY `orders`.`order_id` DESC",
        [uID], (err1, res1) => {
          db.query("SELECT * FROM `product_template` WHERE `extra_cat_id` = ? ORDER BY `product_template`.`temp_name` ASC", [extraCatId], (err2, res2) => {
            if (!err2) {
              db.query("SELECT * FROM `product_temp_video` WHERE `product_temp_id` = ?", [res2[0].temp_id], (err3, res3) => {
                if (err3) {
                ...
                var encImages = tempImages.map((item) => {
                  item.product_temp_id = crypto.smallEncrypt(item.product_temp_id)
                  return item
                })
