  ...
  if (isLogged) {
    try {
      ...
      const [mainCat, subCat, extraCat, allCat, images, currRate] = await Promise.all([
        ...
        db.query("SELECT * FROM `product_faq` WHERE `product_id` = ?", [usepID], (err1, res1)=>{
          if (!err1) {
...
exports.addFaq = (req, res) => {
    ...
    db.query("INSERT INTO `product_faq` (`faq_id`, `product_id`, `question`, `answer`) VALUES (NULL, ?, ?, ?)",
        [usepID, question, answer], (err1, res1)=>{
            if (!err1) {
                res.redirect("back")
            } else {
                res.send(err1)
...
exports.updateFaq = (req, res) => {
    ...
    db.query("UPDATE `product_faq` SET `question` = ?, `answer` = ? WHERE `product_faq`.`faq_id` = ?",
        [qus, ans, fID], (err1, res1)=>{
        if (!err1) {
            res.redirect("back")
        } else {
            res.send(err1)
...
exports.delFaq = (req, res) => {
    var fID = req.params.faq_id
    db.query("DELETE FROM `product_faq` WHERE `product_faq`.`faq_id` = ?", [fID], (err1, res1)=>{
        if (!err1) {
            res.redirect("back")
        } else {
            res.send(err1)
