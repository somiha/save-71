          ...
          if (result[0].phone === 0) {
            ...
            db.query(
              ...
              (err1, res1) => {
                ...
                db.query("SELECT * FROM `orders` WHERE `user_id` = ? AND `in_cart` = 1;", [userId], (err2, res2) => {
                  if (err2) {
                    console.log(err2);
                    res.send("Internal Server Error !");
                    return;
                    ...
                    if (res2.length > 0) {
                      res.cookie("order_id", crypto.encrypt(res2[0].order_id), { maxAge: 24 * 60 * 60 * 1000 });
                    }
                    res.cookie("userId", crypto.encrypt(userId), { maxAge: 24 * 60 * 60 * 1000 });
                    res.cookie("seller_id", crypto.encrypt(res1[0].id), {
                      maxAge: 24 * 60 * 60 * 1000,
                    });
                    if (res1[0].pic_url === 'j:null' || res1[0].pic_url === null || res1[0].pic_url === 'null' || res1[0].pic_url === '') {
                      res.cookie("userImage", crypto.encrypt('null'), {
                        maxAge: 24 * 60 * 60 * 1000,
                      });
                    } else {
                      res.cookie("userImage", crypto.encrypt(res1[0].pic_url == null ? '' : res1[0].pic_url), {
                        maxAge: 24 * 60 * 60 * 1000,
                      });
                    }
                    res.cookie("userName", crypto.encrypt(res1[0].user_name), {
                      maxAge: 24 * 60 * 60 * 1000,
                    });
