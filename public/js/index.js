...
if (window.innerWidth >= 992) {
	subDropdowns.forEach((subDropdown) => {
		subDropdown.addEventListener('mousemove', (e) => {
			subDropdown.children[1].style.display = 'block';
		});
		subDropdown.addEventListener('mouseout', (e) => {
...
if(notificationBtn)
{
			notificationBtn.addEventListener('click', e=>{
				if(notificationContainer.classList.contains('notification-div'))
				{
					notificationContainer.classList.replace('notification-div', 'notification-div-hide');
				}
				else{
					notificationContainer.classList.replace('notification-div-hide','notification-div');
				}
			})
	document.addEventListener('click', e=>{
		if(e.target.parentElement!==notificationBtn)
		{
			notificationContainer.classList.remove('notification-div');
			notificationContainer.classList.add('notification-div-hide');
		}
